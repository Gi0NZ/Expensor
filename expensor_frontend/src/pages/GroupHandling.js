...
const GroupHandling = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const [groupData, groupMembers, expensesData, adminData] = await Promise.all([
  ...
  const handleAddMember = async (e) => {
    ...
    try {
        ...
        if(!usersFound || usersFound.length === 0){
          showError("La mail selezionata non corrisponde ad un utente registrato!");
          return;
        }

        const userToAdd_microsoft_id = usersFound[0].microsoft_id;
        const alrExistsMember = await getSingleGroupMember(groupId, userToAdd_microsoft_id);
  ...
  if (loading) return <div className="group-handling-wrapper"><Navbar /><div className="group-handling-content"><p>Caricamento...</p></div></div>;
  if (!group) return <div className="group-handling-wrapper"><Navbar /><div className="group-handling-content"><p>Gruppo non trovato.</p></div></div>;
  ...
  return (
    <div className="group-handling-wrapper">
      ...
      <div className="group-handling-content">
        ...
        <div className="group-handling-section-row">
          ...
          <div className="group-handling-card group-handling-col-left">
            ...
            <form onSubmit={handleAddExpense}>
              <div className="group-handling-form-group">
                <label className="group-handling-label" htmlFor="description">Descrizione</label>
                <input 
              ...
              <div className="group-handling-form-group">
                <label className="group-handling-label" htmlFor="amount">Importo</label>
                <input 
                    className="group-handling-input"
                    type="number" 
                    value={amount} 
              ...
              <button type="submit" className="group-handling-save-btn">Salva spesa</button>
            </form>
          </div>
          ...
          <div className="group-handling-card group-handling-col-right">
            ...
            <div className="group-handling-list">
              {expenses.length === 0 ? (
                <p style={{color: '#7f8c8d', textAlign: 'center'}}>Nessuna spesa ancora registrata.</p>
              ) : (
                    ...
                    <div className="group-handling-action-wrapper">
                      ...
                      {isUserAdmin && (
                        <button 
                          ...
                          onClick={(e) => handleDeleteGroupExpense(e, expense.id)}
                        >
                          Elimina
                        </button>
                      )}
        ...
        <div className="group-handling-section-row" style={{marginBottom: '40px'}}>

          <div className="group-handling-card group-handling-col-left">
            <h3>➕ Aggiungi Partecipante</h3>
            <form onSubmit={handleAddMember}>
              <div className="group-handling-form-group">
                <label className="group-handling-label" htmlFor="memberEmail">Email Utente</label>
                <input 
                  className="group-handling-input"
                  type="email" 
                  id="memberEmail"
              ...
              <button type="submit" className="group-handling-save-btn">Aggiungi al Gruppo</button>
            </form>
          </div>
          ...
          <div className="group-handling-card group-handling-col-right">
            ...
            <div className="group-handling-list">
              ...
              <div className='group-handling-item' style={{cursor: 'default', backgroundColor: '#fdfdfd', borderLeft: '4px solid #f1c40f'}}>
                <div className='group-handling-item-details'>
              ...
              {members.length === 0 ? (
                <p style={{color: '#7f8c8d', textAlign: 'center'}}>Nessun altro membro nel gruppo.</p>
              ) : (
                members.map(member => (
                  <div key={member.user_id} className="group-handling-item" style={{cursor: 'default'}}> 
                    <div className="group-handling-item-details">
                    ...
                    <div className="group-handling-action-wrapper">
                      {isUserAdmin && (
                        <button 
                          className="group-handling-delete-btn"
                          onClick={(e) => handleDeleteGroupMember(e, member.user_id)}
                        >
                          Espelli
                        </button>
                      )}
